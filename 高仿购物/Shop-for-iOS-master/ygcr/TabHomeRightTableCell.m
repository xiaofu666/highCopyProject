...
- (void)fillContentWithProduct:(ProductEntity *)product
{
    ...
    [_productImage sd_setImageWithURL:[NSURL URLWithString:product.imageSmall] placeholderImage:[UIImage imageNamed:@"image_holder"]];
    ...
    if ([product.featuredPrice floatValue] > 0 && reducePrice > 0) {
        _curPriceLabel.text = [FormatUtil formatPrice:product.featuredPrice];
        _reducePriceLabel.text = [NSString stringWithFormat:@"立减%@元", [FormatUtil formatFloatPrice:reducePrice]];
