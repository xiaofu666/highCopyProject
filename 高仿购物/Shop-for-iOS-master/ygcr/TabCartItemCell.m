...
- (void)fillContentWithCartItem:(CartItemEntity *)cartItem couponUser:(NSArray *)couponUsers totalPrice:(NSNumber *)totalPrice
{
    ...
    [self setIsSelectedBy:cartItem.isSelected];
    
    [_vImage sd_setImageWithURL:[NSURL URLWithString:cartItem.product.imageSmall]];
    ...
    float reducePrice = [cartItem.product.price floatValue] - [cartItem.product.featuredPrice floatValue];
    if ([cartItem.product.featuredPrice floatValue] > 0 && reducePrice > 0) {
        [_vCurPrice setCurPrice:cartItem.product.featuredPrice];
...
- (void)setIsSelectedBy:(NSNumber *)isSelected
{
    ...
    BOOL isSelectedBool = [isSelected boolValue];
    if (isSelectedBool) {
        [self setSelected];
    } else {
        [self setUnselected];
...
- (void)clickSelectBtn
{
    ...
    if ([self.isSelected boolValue]) {
        isSelected = [NSNumber numberWithInt:0];
    } else {
        isSelected = [NSNumber numberWithInt:1];
    }
    [self.delegate doClickSelect:self isSelected:isSelected];
}
