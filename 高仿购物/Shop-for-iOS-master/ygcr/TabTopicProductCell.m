...
- (void)fillContentWithProduct:(ProductEntity *)product tagName:(NSString *)tagName;
{
    if (!product || !product.id) {
        return;
    }
    
    [_vImage sd_setImageWithURL:[NSURL URLWithString:product.imageSmall]];
    _vTitle.text = product.name;
    _vShortDesc.text = product.shortDescription;
    
    float reducePrice = [product.price floatValue] - [product.featuredPrice floatValue];
    if ([product.featuredPrice floatValue] > 0 && reducePrice > 0) {
        [_vCurPrice setCurPrice:product.featuredPrice];
    ...
    if ([@"topSaleTag" isEqualToString:tagName]) {
        _vSaleNum.text = [NSString stringWithFormat:@"已售出%@份", product.totalCount];
        _vReducePrice.text = nil;
    }
}
