...
#import "Union_Video_NewCollectionView.h"
...
@interface Union_Video_NewCollectionView ()<UITableViewDataSource,UITableViewDelegate,GearPoweredDelegate>
...
@implementation Union_Video_NewCollectionView
-(void)dealloc{
...
//        注册cell
        [self registerClass:[NewTableViewCell class] forCellReuseIdentifier:@"CELL"];
...
//请求数据
    ...
    __block Union_Video_NewCollectionView *Self =self;
...
//        在字典中找到key取值  转换成NSInteger
        ...
        newModel.video_length = [[self getStringWithTime:videoLength] retain];
        
        newModel.vid = [[dic valueForKey:@"vid"] retain];
        
        newModel.cover_url = [[dic valueForKey:@"cover_url"] retain];
        
        newModel.title = [[dic valueForKey:@"title"] retain];
        
        newModel.upload_time = [uploadTime retain];
        
        [self.tableArray addObject:newModel];
...
//    给cell的数据模型赋值
    ...
    NSURL *picUrl = [NSURL URLWithString:[self.tableArray[indexPath.row] cover_url]];
