...
#import "zgCommonMacro.h"
#import "zgSCNavTabBar.h"
#import "UIImageView+WebCache.h"
#import "AppDelegate.h"

@interface zgSCNavTabBarController () <UIScrollViewDelegate, zgSCNavTabBarDelegate>
{
    NSInteger       _currentIndex;              // current page index
    NSMutableArray  *_titles;                   // array of children view controller's title
    
    zgSCNavTabBar     *_navTabBar;                // NavTabBar: press item on it to exchange view
    UIScrollView    *_mainView;                 // content view
}

@end

@implementation zgSCNavTabBarController
...
- (void)viewDidLoad
{
    ...
    UIButton *button=[[UIButton alloc]initWithFrame:[delegate createFrimeWithX:10 andY:25 andWidth:50 andHeight:44]];
...
- (void)viewInit
{
    ...
    // 顶部横条滚动视图设置
    _navTabBar = [[zgSCNavTabBar alloc] initWithFrame:CGRectMake(DOT_COORDINATE, DOT_COORDINATE1, SCREEN_WIDTH, NAV_TAB_BAR_HEIGHT) showArrowButton:_showArrowButton];
    _navTabBar.delegate = self;
    _navTabBar.backgroundColor = [UIColor whiteColor];
    _navTabBar.alpha=0.3;
    ...
    // 里面内容视图设置
    _mainView = [[UIScrollView alloc] initWithFrame:CGRectMake(DOT_COORDINATE,_navTabBar.frame.origin.y + _navTabBar.frame.size.height, SCREEN_WIDTH, SCREEN_HEIGHT - _navTabBar.frame.origin.y - _navTabBar.frame.size.height - STATUS_BAR_HEIGHT - NAVIGATION_BAR_HEIGHT + 64)];
